...
if [[ -n "$CODEX" ]]; then
  ...
  if [[ -z "$PIN" ]]; then
    warn "A4 pin: [providers.codex] version_pin is BLANK. Record '$VERSION' in $(advisors_registry_path)"
...
OPTOUT_MARK="$(advisors_optout_marker)"
if [[ -f "$OPTOUT_MARK" ]]; then
  ...
  warn "   mkdir -p '$PROJ_ROOT/.claude' && touch '$PROJ_ROOT/.claude/.codex-training-optout-confirmed' (this repo)"
  warn "   or: mkdir -p ~/.claude && touch ~/.claude/.codex-training-optout-confirmed (all repos)"
fi
