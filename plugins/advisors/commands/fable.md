---
description: Advisory Fable consultation - claude-fable-5 with fresh-spawn + transcript verification
---
...
workbench `/advisors:sol`, `/advisors:terra`, `/advisors:luna` use — so the SAME staged brief
can be run past Fable AND a codex model and their answers compared; `/advisors:consult`
orchestrates that; the bare names — `/sol`, `/consult`, … — resolve only where the bundled
CCMAF commands are installed).
...
- `FABLE_MODEL` = `claude-fable-5` — the required model id (matches `[advisors.fable]` in
  `advisors.toml` — project `.claude/` override, else `~/.claude/`). A config value, not a literal to hardcode in logic; `verify-turn.sh`
...
- If inside a git repo, append `_advisors/` to the repo's **committed** `.gitignore` if absent
  (idempotent; create `.gitignore` if the repo has none). Not in a git repo → skip this bullet.
- `mkdir -p _advisors && cp "${CLAUDE_PLUGIN_ROOT}/fable/verify-turn.sh" _advisors/verify-turn.sh && chmod +x _advisors/verify-turn.sh` — the
...
before); **new** topics — and every `/advisors:consult` seat — go to `_advisors/`. Never migrate a
...
<!-- FABLE-DRIVER: Steps 3-4 below are the single definition of "run a Fable seat". /advisors:consult
     invokes them BY REFERENCE ("execute /advisors:fable Steps 3-4 for advisor fable against <topic>,
     round NNN") — it must NOT restate them, or the two copies drift. -->

## Step 3 — Run one turn (FRESH spawn)  ⟵ the Fable "driver" (by-reference from /advisors:consult)
Spawn a FRESH Fable agent (`model: fable`) with a prompt that:
- pastes the **mandatory guard clause** from `${CLAUDE_PLUGIN_ROOT}/fable/PROTOCOL.md` (skip cold
...
- names its output `_advisors/<topic>/turns/<NNN>-fable.md` (zero-padded next N; when invoked by
  `/advisors:consult`, use the panel round's shared NNN),
- embeds a fresh NONCE.
Do NOT resume a prior Fable agent — resuming drops the model to Opus. Every turn is a new spawn.

## Step 4 — Verify + promote  ⟵ the Fable "driver" (by-reference from /advisors:consult)
