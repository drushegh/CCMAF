---
description: Advisory Sol consultation - gpt-5.6-sol via the codex CLI (ChatGPT sub), verified fail-closed
---
...
subscription (NOT the metered API)**. This is the codex-provider sibling of `/advisors:fable`
(bare `/fable` resolves only where the bundled CCMAF commands are installed), for when
...
## Config
The advisor is defined in the **consumer-owned** registry `advisors.toml` (project `.claude/` override, else `~/.claude/`) (row
`[advisors.sol]` → `provider="codex"`, `model="gpt-5.6-sol"`, `effort`); first use creates it
from the shipped template (Step 0). `${CLAUDE_PLUGIN_ROOT}/scripts/`
ships the driver + verifier + preflight. See `${CLAUDE_PLUGIN_ROOT}/scripts/PROTOCOL.md`.

## Step 0 — Ensure the workbench + registry
- If inside a git repo and `_advisors/` is not git-ignored, append `_advisors/` to the repo's
  **committed** `.gitignore` (idempotent — check first; create `.gitignore` if the repo has
  none). Never commit `_advisors/` contents. Not in a git repo → skip the ignore step.
- Ensure the live registry exists (first use):
  ...
  else `~/.claude/advisors.toml` — and prints the resolved path. (The driver auto-ensures it
  too; this step just makes the location visible.)

## Step 1 — Preflight (NEVER silently downgrade)
Run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/codex-preflight.sh"`.
- **exit 0** → codex is ready. Note any WARNINGs; in particular, if **A5** (training opt-out)
  ...
  their ChatGPT account's training opt-out at chatgpt.com, then attest once (from the repo root):
  `mkdir -p .claude && touch .claude/.codex-training-optout-confirmed` (this repo) or
  `mkdir -p ~/.claude && touch ~/.claude/.codex-training-optout-confirmed` (all repos).
...
- **exit 1 (FAIL)** → tell the user Sol is unavailable and why (paste the failing lines).
  Do NOT fall back to another model silently. Offer: fix the blocker, or use `/advisors:fable` instead.
...
(an `xhigh` consult can exceed the Bash tool's 600s cap — Fable A1):
    ...
    bash "${CLAUDE_PLUGIN_ROOT}/scripts/codex-run.sh" sol <topic> <NONCE>
...
Run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/verify-sol.sh" sol <topic> <NONCE>`.
...
- **FAIL** → discard the turn, tell the user it could not be verified as `gpt-5.6-sol`, paste the
  FAIL reason, and offer to retry or use `/advisors:fable`. NEVER promote or relay an unverified turn.
