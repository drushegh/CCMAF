---
description: Advisory Luna consultation - gpt-5.6-luna via the codex CLI (ChatGPT sub), verified fail-closed
---
...
its sibling `${CLAUDE_PLUGIN_ROOT}/commands/sol.md`, with ONE substitution: the advisor is **`luna`** (not `sol`) — the
model + effort come from `[advisors.luna]` in `advisors.toml` (project `.claude/` override, else `~/.claude/`). Concretely:
...
4. **Run (FRESH, background)** — `bash "${CLAUDE_PLUGIN_ROOT}/scripts/codex-run.sh" luna <topic> <NONCE>` via your Bash tool with `run_in_background: true`. Never `codex exec resume`.
5. **Verify + promote (fail-closed)** — `bash "${CLAUDE_PLUGIN_ROOT}/scripts/verify-sol.sh" luna <topic> <NONCE>`. PASS → promote `.run-luna/last.txt` to `_advisors/<topic>/turns/<NNN>-luna.md`, append to `conversation.md`, relay Luna's take **with the client-attestation caveat**. FAIL → discard, disclose, never relay.
