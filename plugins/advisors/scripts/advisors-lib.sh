...
# Project root (where the _advisors/ workbench lives). git first; outside a
# git repo fall back to the WORKING dir — never walk up from the code dir,
# which in plugin form lives in the plugin cache, not the project.
advisors_repo_root() {
...
# PLUGIN REHOME (v2, TASK-163): the registry resolves project-first, then
# user-level — so the plugin serves arbitrary repos with zero per-project
# files, while a CCMAF project's own .claude/advisors.toml keeps winning
# (byte-identical v1 behaviour there). Bootstrap lands at the resolved path.
advisors_registry_path() {
  local proj
  proj="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
  if [[ -f "$proj/.claude/advisors.toml" ]]; then
    printf '%s/.claude/advisors.toml' "$proj"
  else
    printf '%s/.claude/advisors.toml' "$HOME"
  fi
}

# A5 training-optout marker: same project-first/user-fallback chain. Touching
# the ~/.claude/ marker confirms the opt-out once for every repo on the machine.
advisors_optout_marker() {
  local proj
  proj="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
  if [[ -f "$proj/.claude/.codex-training-optout-confirmed" ]]; then
    printf '%s/.claude/.codex-training-optout-confirmed' "$proj"
  else
    printf '%s/.claude/.codex-training-optout-confirmed' "$HOME"
  fi
}
...
advisors_ensure_registry() {
  ...
  if [[ ! -f "$live" ]]; then
    ...
    if ! mkdir -p "$(dirname "$live")" || ! cp "$template" "$live"; then
      printf 'advisors-lib: ERROR — could not create registry at %s\n' "$live" >&2
      return 1
    fi
