...
> **PLUGIN NOTES (council plugin build):** (1) The council agents ship inside
> this plugin, so their subagent types are namespaced — spawn
> `council:council-contrarian`, `council:council-chairman`, etc. If a
> namespaced type errors as unknown on this Claude Code version, fall back to
> the bare `council-…` name — but note: in a repo that also carries bundled
> CCMAF council agents (the pre-v2.0 dual-install window), the bare name
> resolves to the BUNDLED copies, which may lag the plugin's; prefer the
> namespaced form. (2) The command itself is namespaced too — `/council:council`;
> where this runbook says `/council`, read the namespaced form. (3)
> Deliberations write to `.claude/council/` in the working repo and are local
> working papers, not content to commit. Before Phase 0, ALWAYS verify the
> tree is ignored: `git check-ignore -q .claude/council/probe` (exit 0 =
> ignored, nothing to do). If not ignored, add a local-only exclude:
> `printf '\n%s\n' '.claude/council/' >> "$(git rev-parse --git-path info/exclude)"`
> (creates the file if absent; the leading newline guards against a missing
> trailing newline) and re-run the probe to confirm. Only if you must edit
> `.gitignore` instead, use the same newline guard and tell the user you
...
- `council:council-contrarian`
- `council:council-first-principles`
- `council:council-expansionist`
- `council:council-outsider`
- `council:council-executor`

(If a namespaced type errors as unknown, fall back to the bare `council-…`
names — see PLUGIN NOTES.)
...
Send **one Task tool call** to `council:council-chairman` (bare
`council-chairman` only as the PLUGIN NOTES fallback):
...
The whole `.claude/council/` tree must be git-ignored — deliberations stay
local. The PLUGIN NOTES check (item 3) verifies and, if needed, establishes
that before Phase 0 writes anything; do not skip it.
...
- Do not pass project-layout preambles (e.g. a framework's `## Project paths` block for subagents). Council advisors reason about decisions, not file layouts.
