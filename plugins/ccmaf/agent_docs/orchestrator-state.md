...
- **Over-recovery.** The alternative — re-read the full Cold Start stack (see "Cold Start
  Sequence" — in v2, the SessionStart digest + boot view) on every recovery — reloads tens of KB of mostly-cold
...
The pattern plugs into the four named moments in the session digest's "Session lifecycle"
...
- Give the verifier the doer's claim AND the instruction to try to refute it — a verifier
  ...
  never the acceptance gate (the session digest "Agents" rules).
