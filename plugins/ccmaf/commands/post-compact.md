...
- **Present without `FRAMEWORK_LINE=v2`** → say "this is a v1-line CCMAF
  project — use the bundled /post-compact or migrate" and stop.
...
Per the **Re-anchor / Reconcile** rule (`${CLAUDE_PLUGIN_ROOT}/DIGEST.md` "Session Lifecycle") —
RECONCILE, don't blindly trust either side:

**If the project uses the orchestrator-state pattern** (`.claude/NOW.md` exists — see
`${CLAUDE_PLUGIN_ROOT}/agent_docs/orchestrator-state.md`), start with its boot set, in order:
