...
- **Present without `FRAMEWORK_LINE=v2`** → say "this is a v1-line CCMAF
  project — use the bundled /wrapup or migrate" and stop.
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/insights/instinct-miner.sh
```
...
## 9. Stop the project Console (session end)

`/wrapup` is the session-END boundary, so tear down this project's Console if it
was opted in (`.claude/.console-version` or the legacy `.claude/.console-enabled`):

- **Opted in AND the `console:stop` plugin command resolves** → invoke it (Skill
  tool, skill `console:stop`). This gracefully shuts the project's Console down
  (the tray Hub drops it, and the Hub exits when its last console closes —
  `contract:console-lifecycle`).
- **Opted in but the command does not resolve** (console plugin not installed) →
  skip with a note; the heartbeat reap-grace covers it, so this is safe to skip
  rather than reach for an install path.
- **Not opted in, or no console running** → no-op.

Never reference `tools/console.mjs` or any install path directly — the seam is
the plugin command, by name, double-gated on opt-in + resolution. Do **not**
tear the console down in `/pre-compact` — that is a mid-session Checkpoint, not
a session end, and the Console must stay up across a manual compaction. Report
the result on one line (✓ stopped / — not opted in / — plugin absent, skipped
/ — none running).
...
**Unattended runs:** when `CLAUDE_UNATTENDED=1` is set, additionally emit one
line summarising the run before the verdict below: "N tasks done, M Blocked
with questions, K defaults logged (see `.claude/unattended-log.md`)."
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/insights/session-summary.sh
bash ${CLAUDE_PLUGIN_ROOT}/insights/push-state.sh
```
...
Then, after all nine steps, emit one of:
