...
- **Present without `FRAMEWORK_LINE=v2`** → say "this is a v1-line CCMAF
  project — use the bundled /security or migrate" and stop.
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/audit/config-security.sh
```
...
5. **CLAUDE.md / AGENTS.md** — prompt-injection
...
- **No project code (framework-only repo / fresh clone).** Parts 2, 3, 5
