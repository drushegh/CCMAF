...
- **Present without `FRAMEWORK_LINE=v2`** → say "this is a v1-line CCMAF
  project — use the bundled /healthcheck or migrate" and stop.
...
**1.1. Fast invariants.** Run `bash ${CLAUDE_PLUGIN_ROOT}/doctor/doctor.sh`.
Capture any findings from `.framework-doctor-findings.md`.

**1.2. Behavioural drift.** Run `bash ${CLAUDE_PLUGIN_ROOT}/insights/analyse.sh --force`
and `bash ${CLAUDE_PLUGIN_ROOT}/insights/report.sh`. Capture `.framework-insight-alert.md`
if it appears; include the report summary either way.

**1.3. Recent framework change detection.** In v2, core's own code and prose
are machine-wide plugin content (versioned via `claude plugin update`), not
tracked in this project's git history — so a project-local `git diff` can no
longer see an in-place framework code change the way v1's diff did. It CAN
still see project-scaffold drift (custom agents, bare-alias command files,
the `SCAFFOLD_REV` pin). Check for changes in the last
`HEALTHCHECK_FW_WINDOW` commits (default 10) across what remains in-project:

```bash
git diff --name-only HEAD~${HEALTHCHECK_FW_WINDOW:-10} -- \
  .claude/agents/ .claude/commands/ .claude/.framework-version 2>/dev/null
```

**If changes detected → deep framework audit via reviewer subagent:**

Spawn a **`ccmaf:reviewer`** subagent (fall back to the bare `reviewer`
name if the namespaced type does not resolve) with this prompt. Prepend
the shape detected in Part 0 so the subagent doesn't re-derive it
(scoped-anchor rule — shape context only, never findings):

> **Detected shape (from Part 0):** source dirs: `<HEALTHCHECK_SOURCE_DIRS
> or "none — framework-only">`; state root: `.claude/`; contracts:
...
> - `${CLAUDE_PLUGIN_ROOT}/agent_docs/OPERATIONS.md` references → resolve?
...
> - Commands referencing plugin paths (`${CLAUDE_PLUGIN_ROOT}/agent_docs/`,
>   `${CLAUDE_PLUGIN_ROOT}/doctor/`, etc.) → paths resolve inside the
>   installed plugin? Commands referencing project-relative paths
>   (`docs/`, `contracts/`) → exist in the project?
> - Agent definitions referencing `${CLAUDE_PLUGIN_ROOT}/agent_docs/`
>   files → those files exist?
> - `settings.json` — in v2 it carries permissions only (hooks arrive via
>   plugins, not settings.json registrations) — confirm it still parses
>   as valid JSON and its permissions block is well-formed; it should NOT
>   contain hook-command paths to verify.
> - Also run `bash ${CLAUDE_PLUGIN_ROOT}/doctor/doctor.sh` if not already
>   run and fold its findings in.
>
...
> - Command instructions — do any contradict the core plugin's digest
>   (`${CLAUDE_PLUGIN_ROOT}/DIGEST.md`) rules?
...
Spawn a **`ccmaf:reviewer`** subagent (fall back to the bare `reviewer`
name if the namespaced type does not resolve). Prompt template (fill in
...
Spawn a **`ccmaf:reconciler`** subagent (fall back to the bare
`reconciler` name if the namespaced type does not resolve), `full` mode.
Prompt:
...
Spawn a **`ccmaf:reviewer`** subagent (fall back to the bare `reviewer`
name if the namespaced type does not resolve). Prompt:
