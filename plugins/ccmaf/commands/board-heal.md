...
- **Present without `FRAMEWORK_LINE=v2`** → say "this is a v1-line CCMAF
  project — use the bundled /board-heal or migrate" and stop.
...
`${CLAUDE_PLUGIN_ROOT}/agent_docs/verify-handback.md` (the seed schema + the retroactive/decompose
...
- Re-run `bash ${CLAUDE_PLUGIN_ROOT}/doctor/doctor.sh` and confirm the `board-coherence` / suffixed-ID
