...
Complementary to [`${CLAUDE_PLUGIN_ROOT}/update/`](../update/) (pulls framework
improvements) and [`${CLAUDE_PLUGIN_ROOT}/doctor/`](../doctor/) (point-in-time
...
| [update-metrics.sh](update-metrics.sh) | Refreshes the auto-generated snapshot block in `.claude/framework-metrics.md` (project root). Run on demand or at session end. |
...
- Bash hooks emit via `telemetry_emit()` in the plugin's own
  `hooks/lib/hook-common.sh` copy (v2 adaptation — v1 shipped this at the
  consumer-bundled `.claude/hooks/lib/hook-common.sh`; core now carries its
  own copy per CORE-DESIGN §1, referenced as
  `${CLAUDE_PLUGIN_ROOT}/hooks/lib/hook-common.sh`).
...
- `.claude/framework-metrics.md` at project root
  is **committed** — it's the human-readable rollup, updated every
  ~10 sessions and shared across the team.

## Manual operations

*(v2 adaptation: these scripts now ship inside the ccmaf plugin.
`${CLAUDE_PLUGIN_ROOT}` resolves automatically in hook/command
invocation contexts; for an interactive manual run, locate the plugin
cache dir first — see the top-level plugin README.)*

```bash
# See current state any time
bash "${CLAUDE_PLUGIN_ROOT}/insights/report.sh"

# Force an analysis regardless of throttle
bash "${CLAUDE_PLUGIN_ROOT}/insights/analyse.sh" --force

# Rollup only (no thresholds check)
bash "${CLAUDE_PLUGIN_ROOT}/insights/rollup.sh"

# Dismiss a current alert
rm .claude/.framework-insight-alert.md

# Tune thresholds for your workflow
$EDITOR "${CLAUDE_PLUGIN_ROOT}/insights/thresholds.conf"
```
