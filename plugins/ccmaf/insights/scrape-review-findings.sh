...
# v2 adaptation: hook-common.sh is core's own copy, shipped as an
# own-machinery sibling at ${CLAUDE_PLUGIN_ROOT}/hooks/lib/hook-common.sh
# (CORE-DESIGN §1) — the old path was the consumer-bundled
# .claude/hooks/lib/hook-common.sh, which no longer exists in a v2 project.
_lib="${CLAUDE_PLUGIN_ROOT:-}/hooks/lib/hook-common.sh"
# shellcheck disable=SC1090
[ -n "${CLAUDE_PLUGIN_ROOT:-}" ] && [ -f "$_lib" ] && . "$_lib" || true
