#!/bin/bash
# start-project.sh — Detect stack, install dependencies, optionally start dev server.
# Called during Cold Start to verify the project is in a working state.
# (v2 adaptation: renamed from the v1 `.claude/framework/init.sh` per
# CORE-DESIGN §1 — the old name now belongs to `/ccmaf:init`, the NEW
# scaffold-writing command.)
#
...
# v2 adaptation: CLAUDE_PROJECT_DIR (harness-provided) is now the primary
# source of truth, with the original `git rev-parse --show-toplevel || pwd`
...
# v2 adaptation: skills-sync.sh is an own-machinery sibling, now plugin-side
# (${CLAUDE_PLUGIN_ROOT}/update/skills-sync.sh) rather than the consumer-
# bundled .claude/framework/update/skills-sync.sh.
if [ -f ".claude/.skills-version" ] && [ -n "${CLAUDE_PLUGIN_ROOT:-}" ] && [ -f "${CLAUDE_PLUGIN_ROOT}/update/skills-sync.sh" ]; then
  bash "${CLAUDE_PLUGIN_ROOT}/update/skills-sync.sh" --suggest 2>/dev/null || true
fi
