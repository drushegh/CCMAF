...
#
# Runs as part of cold start (alongside update-nudge.sh). Silent when the
...
#
# Mirrors update-nudge.sh deliberately — same throttle, same DA-C4 flag
...
# v2 adaptation: the old fallback (SCRIPT_DIR/../../..) assumed a fixed depth
...
# non-hook invocation. SCRIPT_DIR itself is unaffected (dirname-based sibling
# lookups below, e.g. skills-sync.sh, stay correct regardless of install
# location).
...
# ${CLAUDE_PLUGIN_ROOT} is literal text to anything reading a persisted flag
# file later — the var exists only inside a hook/command invocation, not in
# the model's Bash tool or a human's terminal (core-context.sh:48-50 sed-
# substitutes the digest for exactly this reason). Bake in the real resolved
# command at write time instead: prefer the env var's value (matches its
# canonical form elsewhere), fall back to the sibling path we already know
# (SCRIPT_DIR), and only fall back to a resolve-it-yourself hint if somehow
# neither is available.
skills_sync_cmd() {
  if [ -n "${CLAUDE_PLUGIN_ROOT:-}" ]; then
    printf 'bash "%s/update/skills-sync.sh"' "$CLAUDE_PLUGIN_ROOT"
  elif [ -f "$SCRIPT_DIR/skills-sync.sh" ]; then
    printf 'bash "%s/skills-sync.sh"' "$SCRIPT_DIR"
  else
    printf '%s' "the ccmaf plugin's update/skills-sync.sh (resolve via claude plugin list --json installPath)"
  fi
}
...
then run: \`$(skills_sync_cmd)\` — it fetches the selected
...
# + authenticated SSH + local paths). timeout-bounded per the standing
# "every network-capable call in the cold-start chain is bounded" rule
# (matches update-nudge.sh's `claude plugin list` bound) so a filtered/dead
# network can never hang session start.
if ! latest_sha=$(timeout 10 git ls-remote "$SKILLS_UPSTREAM_URL" "refs/heads/$SKILLS_UPSTREAM_BRANCH" 2>/dev/null | awk '{print $1}'); then
