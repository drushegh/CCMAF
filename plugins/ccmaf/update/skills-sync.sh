...
#
# Usage:
#   # v2 adaptation: this script now ships inside the ccmaf plugin.
#   # ${CLAUDE_PLUGIN_ROOT} resolves automatically in hook/command invocation
#   # contexts; for an interactive manual run, locate the plugin cache dir
#   # first (see the top-level plugin README).
#   bash "${CLAUDE_PLUGIN_ROOT}/update/skills-sync.sh"             # sync selected skills
#   bash "${CLAUDE_PLUGIN_ROOT}/update/skills-sync.sh" --suggest   # stack-detect suggestions only
#
...
# derive PROJECT_ROOT via a "$SCRIPT_DIR/../../.." climb — broken under the
# plugin layout (fixed-depth assumption). This script has no sibling-file
# lookup, so SCRIPT_DIR is dropped entirely; CLAUDE_PROJECT_DIR
# (harness-provided) is now PROJECT_ROOT's primary source, with git/cwd as
# fallbacks for a manual, non-hook invocation.
PROJECT_ROOT="${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
VERSION_FILE="$PROJECT_ROOT/.claude/.skills-version"
SKILLS_DIR="$PROJECT_ROOT/.claude/skills"

# This script's own real path, for the "re-run me" hint below. ${CLAUDE_PLUGIN_ROOT}
# is literal text outside a hook/command invocation (e.g. a human re-running
# this from a terminal after reading that hint) — BASH_SOURCE resolves
# correctly regardless of how the script was invoked.
...
EOF
  echo "then re-run: bash \"$SCRIPT_SELF\"" >&2
  cat >&2 <<'EOF'
