...
# CLAUDE_PLUGIN_ROOT (devhooks/console precedent — each plugin carries an
# independent copy, drift-guarded later) instead of resolving it from this
# script's own on-disk location (which would be the plugin install dir, not
# the project). Inline fallback definition of core_active covers the lib
# failing to load — same shape as the sibling core-plugin hooks
# (core-context.sh / task-budget-counter.sh / unattended-guard.sh).
...
DIRECTIVE='⚓ Context was just COMPACTED — the summary above may be lossy or wrong. Before continuing, RE-ANCHOR per the CCMAF session digest "Session lifecycle": (1) re-read your state files (TASKS, STATUS, claude-progress — including any `## WIP` section) as canonical for COMMITTED facts; (2) run `git status` / `git diff` to see actual in-flight work; (3) RECONCILE the summary against disk + the live repo — trust the files and the working tree over the summary where they disagree; (4) do not treat the summary'"'"'s task statuses or decisions as authoritative until checked.'
