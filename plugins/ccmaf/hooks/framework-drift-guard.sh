...
# unattended-guard.sh). Gated as the FIRST effective act — ahead of the
# REPO_ROOT git call below and every other step — so core's copy never even
# spawns a git process in a v1 project or non-CCMAF repo (CORE-DESIGN §0);
# core_active only needs CLAUDE_PROJECT_DIR, not a resolved git root, so it
# can run before REPO_ROOT is known.
...
# State-file regex (STATE_PREFIX is always .claude/; framework-self redirect retired)
...
# already lives in agent_docs/OPERATIONS.md (Framework Feedback), so this fires only
...
if [ "$COMPACT_ENABLED" = true ] && [ "$SINCE_COMPACT" -ge "$COMPACT_TURNS" ]; then
  REMINDERS="${REMINDERS}🧭 Context check: ${PROMPT_COUNT} prompts this session. At a logical breakpoint (a task just finished, about to start something new), consider \`/compact\` or a fresh session to keep context lean — see the CCMAF session digest (context-awareness rules). When compacting, preserve modified files, task status, and current decisions.\n"
  DRIFT_DETECTED=true
