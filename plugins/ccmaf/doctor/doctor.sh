...
#
# Complementary to (v2 adaptation: own-machinery siblings, now plugin-side):
#   - ${CLAUDE_PLUGIN_ROOT}/update/ (pulls improvements)
#   - ${CLAUDE_PLUGIN_ROOT}/insights/ (longitudinal efficacy)
#
...
# v2 adaptation: the old fallback (a BASH_SOURCE-relative
# SCRIPT_DIR="$(dirname ...)" then PROJECT_ROOT="$SCRIPT_DIR/../../..")
# assumed a fixed depth under a consumer's .claude/framework/doctor/ tree —
# under the plugin layout (plugins/ccmaf/doctor/doctor.sh) that climb lands
# inside the plugin cache, not the project. CLAUDE_PROJECT_DIR
# (harness-provided) is now the primary source of truth; git/cwd remain
# fallbacks for a manual, non-hook invocation.
PROJECT_ROOT="${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
FLAG_FILE="$PROJECT_ROOT/.claude/.framework-doctor-findings.md"
# v2 adaptation: framework-manifest.txt was doctor's own-machinery sibling in
# v1 (both lived under .claude/framework/). It is NOT in Wave A1's port list
# (see the update/ port scope) and the file itself is v1-line-only per
# CORE-DESIGN §1 (retired at v2.1). The path below still follows the
# own-machinery-sibling rule (${CLAUDE_PLUGIN_ROOT}/update/...) so Check 4
# degrades cleanly to one INFO finding when the file is absent, rather than
# silently resolving into the wrong tree.
MANIFEST_FILE="${CLAUDE_PLUGIN_ROOT:-}/update/framework-manifest.txt"
SETTINGS_FILE="$PROJECT_ROOT/.claude/settings.json"
VERSION_FILE="$PROJECT_ROOT/.claude/.framework-version"

# v2 adaptation: several checks branch on the framework LINE (CORE-DESIGN §0
# / contract:core-plugin-v2). Whitespace-tolerant, CR-tolerant (no $ anchor).
_is_v2_line() {
  [ -f "$VERSION_FILE" ] && grep -Eq '^[[:space:]]*FRAMEWORK_LINE=v2' "$VERSION_FILE" 2>/dev/null
}

# v2 adaptation (round-2 review P1, live-reproduced): the flag lifecycle is
# COMPLETION-GATED, never delete-first. The old `rm -f` here meant a doctor
# killed mid-run (cold-start wraps it in `timeout`) erased a live CRITICAL
# flag — after which the unattended halt machinery (unattended-guard,
# enforce-state exemption, core-context halt-clear) all read flag-absence as
# "clean" and failed OPEN. Now: the old flag persists untouched throughout
# the run; a COMPLETED clean run removes it at the end; a findings run
# tmp+mv's the new flag atomically over it. A killed run leaves the last
# completed verdict in place — stale-but-safe, never absent-but-critical.
...
_dnow_ms() { local t; t=$(date +%s%3N 2>/dev/null); case "$t" in [0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]) printf '%s' "$t" ;; *) printf '%s' "$(( $(date +%s) * 1000 ))" ;; esac; }
_run() {
...
check_cross_refs() {
  ...
  # v2 adaptation: agents/ and agent_docs/ are own-machinery, now plugin-side
  # (CORE-DESIGN §1) rather than consumer-bundled .claude/agents/framework +
  # .claude/framework/agent_docs. [ASSUMED, unverified against Wave A1]: the
  # paths below assume agents/ and agent_docs/ sit directly under the plugin
  # root (no /framework subdir) — confirm against whichever wave lands
  # agents/ + agent_docs/ under plugins/ccmaf/, and adjust if that wave uses
  # a different layout.
  while IFS= read -r f; do
    scan_files+=("$f")
  done < <(find "${CLAUDE_PLUGIN_ROOT:-}/agents" "${CLAUDE_PLUGIN_ROOT:-}/agent_docs" -maxdepth 3 -type f -name '*.md' 2>/dev/null)
...
check_claude_include() {
  ...
  # v2 adaptation: on the v2 line the standing rules arrive via the
  # SessionStart digest — CLAUDE.md deliberately carries NO framework
  # include, and a lingering CLAUDE.framework.md is migration residue, not
  # a broken pointer. Demanding the @import here would false-CRITICAL every
  # v2 project (and trip the unattended halt).
  _is_v2_line && return
...
check_manifest_paths() {
  if [ ! -f "$MANIFEST_FILE" ]; then
    # v2 adaptation (review swarm P1): on a v2-line project the manifest is
    # retired BY DESIGN — an unconditional INFO here dirties every scan
    # ("silent if clean" broken) and writes a flag file on healthy projects.
    # v2 → silent; v1 with the manifest genuinely missing → real INFO.
    _is_v2_line && return
    add_finding "INFO" "manifest" "framework-manifest.txt not found (v1-line project without the bundled update system? MIGRATION.md unported). The v1 updater cannot run without it."
    return
  fi
...
# --- Check 4.5: kernel presence (D1-a's compensating control) --------
# The core deliberately ships NO guard copy (owner ruling D1-a: the kernel
# IS the safety floor). This check is the ruled compensating control the
# review swarm found missing (its P0): a v2 project whose ccmaf-kernel was
# disabled/uninstalled loses dangerous-command screening with zero signal.
# Evidence, fail-safe ladder:
#   - marker fresh (.claude/telemetry/.kernel-present, touched by
#     kernel-context.sh each active session) -> healthy, silent.
#   - marker missing/stale + `claude plugin list --json` (S3-probed) says
#     ccmaf-kernel is NOT enabled -> CRITICAL (this is the halt-worthy case).
#   - marker missing/stale + the CLI probe unavailable -> WARNING (loud but
#     not halt-tripping: first-session ordering or a PATH-less environment
#     can't be distinguished from a real absence without the probe).
...
check_kernel_present() {
  ...
  if [ -f "$marker" ] && [ -z "$(find "$marker" -mtime +7 2>/dev/null)" ]; then
    return
  fi

  local probe=""
  if command -v claude >/dev/null 2>&1; then
    # timeout 5, not 10: this inner probe runs INSIDE cold-start's outer
    # doctor budget — a slow CLI must not eat the whole slot and get doctor
    # killed (the round-2 live repro's trigger).
    probe="$(timeout 5 claude plugin list --json 2>/dev/null || true)"
  fi
  if [ -n "$probe" ]; then
    if printf '%s' "$probe" | grep -q '"ccmaf-kernel@' \
       && printf '%s' "$probe" | tr -d ' \n' | grep -q '"id":"ccmaf-kernel@ccmaf"[^}]*"enabled":true'; then
      # Installed + enabled; the stale marker is an ordering artifact.
      return
    fi
    add_finding "CRITICAL" "kernel-absent" "This is a CCMAF v2 project but the ccmaf-kernel plugin — the dangerous-command guard, the safety floor the core deliberately does not bundle (D1-a) — is not installed/enabled (plugin-list probe + no fresh .kernel-present marker). Every Bash command is running UNSCREENED. Fix: \`claude plugin install ccmaf-kernel@ccmaf --scope user\` (or re-enable it), then restart the session."
  else
    add_finding "WARNING" "kernel-absent" "No fresh .claude/telemetry/.kernel-present marker and the \`claude plugin list\` probe is unavailable — cannot confirm the ccmaf-kernel plugin (the safety floor) is active. If Bash commands are not being screened, install it: \`claude plugin install ccmaf-kernel@ccmaf --scope user\`."
  fi
}
...
check_framework_version() {
  ...
  # v2 adaptation (CORE-DESIGN §0 / contract:core-plugin-v2): the file has
  # two valid shapes. A v2-line file carries FRAMEWORK_LINE=v2 +
  # SCAFFOLD_REV=<integer> and NONE of the v1 pin fields — validating it
  # against the v1 schema would false-CRITICAL every real v2 project (and,
  # via the unattended-guard's CRITICAL halt, freeze every unattended run).
  local _line
  _line="$(_ver_get FRAMEWORK_LINE)"; _line="${_line%$'\r'}"
  if [ "$_line" = "v2" ]; then
    local _rev
    _rev="$(_ver_get SCAFFOLD_REV)"; _rev="${_rev%$'\r'}"
    case "$_rev" in
      ''|*[!0-9]*)
        add_finding "CRITICAL" "framework-version" ".framework-version declares FRAMEWORK_LINE=v2 but SCAFFOLD_REV is missing or non-numeric ('$_rev'). /ccmaf:update cannot sync the scaffold. Fix: set SCAFFOLD_REV=<integer> (see ${CLAUDE_PLUGIN_ROOT:-the ccmaf plugin}/templates/framework-version)."
        ;;
    esac
    rm -f "$missing_tmp"
    return
  ...
  if [ -s "$missing_tmp" ]; then
    local missing_csv
    missing_csv=$(tr '\n' ',' < "$missing_tmp" | sed 's/,$//')
    add_finding "CRITICAL" "framework-version" ".framework-version is missing required fields: $missing_csv (v1 line — no FRAMEWORK_LINE=v2 present). Updates will not work. Fix: re-run the v1 updater's init-framework-version.sh --force, or migrate to v2."
  fi
  rm -f "$missing_tmp"
}
...
check_detector_consumers() {
  ...
  #
  # v2 GAP (flagged, left UNSWAPPED — not a mechanical path fix): this
  # check's premise is git-log HISTORY of the CONSUMER's own repo. Under
  # v2, insights/ and audit/ are plugin code living in a SEPARATE repo (the
  # ccmaf marketplace) — no per-consumer `git log` can ever see their
  # commits, so swapping these paths to ${CLAUDE_PLUGIN_ROOT} would not make
  # the check work (git -C "$PROJECT_ROOT" cannot see history outside its
  # own tree); it would just no-op for a different reason. Left pointing at
  # the v1 consumer paths (naturally empty/no-op on a v2 project, since they
  # never existed in ITS history) rather than guessing a swap that cannot
  # function either way. A real BUG-001 defense for plugin-owned code
  # belongs in the plugin repo's own CI, not per-consumer doctor.
  local scan_paths=(
...
check_state_budgets() {
  # v2 adaptation: state-budgets.conf is doctor's own-machinery sibling (same
  # directory, both under .claude/framework/doctor/ in v1) — now plugin-side
  # at ${CLAUDE_PLUGIN_ROOT}/doctor/state-budgets.conf. The consumer override
  # stays project-relative (unchanged).
  local conf="${CLAUDE_PLUGIN_ROOT:-}/doctor/state-budgets.conf"
  ...
  while IFS= read -r line; do
    ...
    pct=$(( sz * 100 / budget ))
    if [ "$pct" -ge 100 ]; then
      add_finding "CRITICAL" "state-budget" "\`$rel\` is ${sz}B, OVER its ${budget}B budget (${pct}%) — boot reads and board loading are at risk. Fix: run /ccmaf:housekeeping (\`bash \"${CLAUDE_PLUGIN_ROOT:-<ccmaf-plugin>}/housekeeping/rotate-state.sh\"\`) to archive old entries."
    elif [ "$pct" -ge 80 ]; then
      add_finding "WARNING" "state-budget" "\`$rel\` is ${sz}B, ${pct}% of its ${budget}B budget — approaching the ceiling. Fix: run /ccmaf:housekeeping soon (\`bash \"${CLAUDE_PLUGIN_ROOT:-<ccmaf-plugin>}/housekeeping/rotate-state.sh\"\`)."
    fi
  done < "$conf"
}
...
if [ ${#findings[@]} -eq 0 ]; then
  # COMPLETED clean run: only now is removing the flag safe (see the
  # lifecycle note at the top — a killed run must never leave it absent).
  rm -f "$FLAG_FILE"
  exit 0
fi
...
# Atomic replace: a reader (unattended-guard mid-tool-call) must never see a
# half-written or momentarily-absent flag.
mv -f "${FLAG_FILE}.tmp.$$" "$FLAG_FILE" 2>/dev/null || rm -f "${FLAG_FILE}.tmp.$$"
