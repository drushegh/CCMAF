...
```bash
bash "${CLAUDE_PLUGIN_ROOT}/audit/pattern-scan.sh"
```
...
- Scans `${CLAUDE_PLUGIN_ROOT}` (the plugin's own shell scripts) plus
  `.claude/hooks/` and `.claude/skills/` (project-relative — consumer
  state, unaffected by where the plugin itself is installed). *(v2
  adaptation: v1 scanned the consumer-bundled `.claude/framework/` tree;
  that tree no longer exists post-migration — the framework's own shell
  now lives in the plugin cache. `pattern-scan.sh` falls back to the old
  `.claude/framework` path when `CLAUDE_PLUGIN_ROOT` is unset, e.g. a
  manual run against this framework's own dev repo.)*
- Excludes `pattern-scan.sh` itself — the script's
