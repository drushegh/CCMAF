...
# v2 adaptation: "framework-owned shell scripts" now means the PLUGIN's own
# scripts (this repo no longer carries a project-local .claude/framework/
# tree) — scan ${CLAUDE_PLUGIN_ROOT} instead. Falls back to the old
# .claude/framework path so a manual run from a v1 project (or from this
# framework's own dev repo, which still carries that tree) keeps working.
# .claude/hooks and .claude/skills stay project-relative — project state,
# unaffected by the plugin relocation. CLAUDE_PLUGIN_ROOT is populated in
# hook/command invocation contexts; an interactive manual run may need to
# pass it explicitly (see the plugin README for locating the cache path).
if [ -n "${CLAUDE_PLUGIN_ROOT:-}" ]; then
  SCAN_PATHS=("$CLAUDE_PLUGIN_ROOT" ".claude/hooks" ".claude/skills")
else
...
_sh_files() {
  ...
  for p in "${paths[@]}"; do
    ...
    while IFS= read -r f; do
      # Matches both the absolute plugin-cache form (.../audit/pattern-scan.sh)
      # and the relative v1-fallback form (.claude/framework/audit/pattern-scan.sh)
      # — both end in "/audit/pattern-scan.sh".
      case "$f" in
        *"/audit/pattern-scan.sh") continue ;;
      esac
      found+=("$f")
