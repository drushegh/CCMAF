...
#
# Surfaces scanned (all PROJECT-relative — this audits the CONSUMER's own
# config, never the plugin's own machinery, so none of these paths change
...
#
# Usage:
#   # v2 adaptation: this script now ships inside the ccmaf plugin; interactively
#   # it lives under your Claude Code plugin cache (see the plugin's README) —
#   # in hook/command contexts ${CLAUDE_PLUGIN_ROOT} resolves it directly.
#   bash "${CLAUDE_PLUGIN_ROOT}/audit/config-security.sh" [--format text|json]
#
...
# v2 adaptation: unchanged — REPO_ROOT already resolved via git rev-parse (not
# BASH_SOURCE), so it is unaffected by the plugin-cache relocation. Every
# surface below is project state (the consumer's own config), never plugin
# machinery, so no path here needs a ${CLAUDE_PLUGIN_ROOT} swap.
