 ...
 * launcher verb for THIS project. Shell-agnostic (PowerShell, cmd, bash). Plugin form —
 * the script lives in the plugin cache, so invoke it via its plugin path (in hook
 * commands `${CLAUDE_PLUGIN_ROOT}` expands to that cache dir):
 *
 *   node "$CLAUDE_PLUGIN_ROOT/scripts/console.mjs" start [--root <project>]    # ensure up, print URL (idempotent)
 *   node "$CLAUDE_PLUGIN_ROOT/scripts/console.mjs" stop  [--root <project>]    # /wrapup: graceful teardown
 *   node "$CLAUDE_PLUGIN_ROOT/scripts/console.mjs" open  [--root <project>]    # open it (browser per settings)
 *   node "$CLAUDE_PLUGIN_ROOT/scripts/console.mjs" restart [--root <project>]  # stop then start fresh — after a Console version bump
 *
...
// Plugin copy: this driver lives in the PLUGIN CACHE (${CLAUDE_PLUGIN_ROOT}/scripts/),
// NOT under <project>/tools/, so the bundled copy's script-location-relative
// resolution (resolve(dirname(script), "..")) would yield the plugin cache dir —
// wrong board served, mis-rooted registry entry the heartbeat never touches, and
// a silently ignored .console-version pin. Resolve the PROJECT root from the
// invocation context instead: an explicit `--root <path>` argument (the autostart
// hook passes it), else CLAUDE_PROJECT_DIR (exported to hook processes), else the
// enclosing git toplevel from cwd, else cwd itself.
function resolveRepoRoot() {
  ...
  if (r.status === 0 && r.stdout && r.stdout.trim()) return resolve(r.stdout.trim());
  return process.cwd();
}
...
if (!["start", "stop", "open", "restart"].includes(VERB)) {
  die(`unknown verb "${VERB}" — usage: node "$CLAUDE_PLUGIN_ROOT/scripts/console.mjs" <start|stop|open|restart> [--root <project-root>]`);
}
