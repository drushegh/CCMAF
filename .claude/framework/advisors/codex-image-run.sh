...
# ── Registry resolution (codex-only) ─────────────────────────────────────────────────────
...
if [[ "$provider" != "codex" ]]; then
  status "ERROR advisor '$advisor' is provider '$provider', not codex — image gen needs a codex model"; exit 2
fi
if [[ -z "$model" ]]; then
...
if [[ ! -d "$real_home/skills/.system/imagegen" && ! -e "$real_home/skills/.system/imagegen/SKILL.md" ]]; then
...
if [[ -z "$saved" ]]; then
  ...
  newest="$(find "$real_home/generated_images" -type f \( -iname '*.png' -o -iname '*.webp' -o -iname '*.jpg' \) 2>/dev/null | sort | tail -1)"
