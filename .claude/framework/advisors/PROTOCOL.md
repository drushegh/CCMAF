...
Defined in the **consumer-owned** registry `.claude/advisors.toml` (created from
`advisors.template.toml`; NOT in the framework manifest, so edits survive updates). One row per
advisor: `provider` · `model` · `effort` (provider-native vocabulary — no unified scale).
...
1. **Workbench** — `_advisors/<topic>/conversation.md` (staged brief) + `turns/NNN-<advisor>.md`
   ...
   and their answers diffed — comparative consultation). Git-ignored. (`/fable` still uses
   `_fable/` until it is folded in.)
...
**Config** — `[modes]` in `.claude/advisors.toml`:
