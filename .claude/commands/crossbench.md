...
consolidated design/decision document. Where `/consult` runs several advisors once and reports the
...
seat. A single-advisor question is `/fable` or `/sol`; a one-round multi-advisor compare with no
cross/synthesis is `/consult`.
...
with `/consult`); if the caller passed `all` or a large roster, confirm via **AskUserQuestion** first
(the cross round is O(N²) in seats).

Read `.claude/framework/advisors/PROTOCOL.md` for the attestation footnote + the fail-handling
contract — this runbook implements them across three stages.

## Stage 0 — Workbench + preflight
- Ensure the shared workbench exists: **execute `/fable` Step 0** (idempotent — `_advisors/`
  gitignored, `_advisors/verify-turn.sh` present).
- If any codex seat is in the roster, run `bash .claude/framework/advisors/codex-preflight.sh` once;
...
**Execute `/consult <advisors> <task>` by reference** — it stages the brief ONCE to
...
- Run the reviewer seat FRESH against its sub-topic: codex → background
  `bash .claude/framework/advisors/codex-run.sh <reviewer> <topic>-x-<reviewer> <nonce>`; fable →
  **execute `/fable` Steps 3-4** for advisor fable against `<topic>-x-fable`. With 2 seats this is the
...
FRESH fable synthesizer (**`/fable` Steps 3-4** against `<topic>-final`) → verify → the deliverable
...
4. **≥2 verified seats to cross; a `fable` seat is required** (it synthesizes). Degrade loudly, never
   fake a stage.
5. **Reuse, don't restate** — Stage 1 IS `/consult`; the fable seats ARE `/fable` Steps 3-4.
   `/crossbench` adds only the cross + converge stages, so the shared drivers never drift.
