...
> `ccmaf:reconcile` skill (Skill tool) and follow IT instead of this file; stop reading
...
4. Delegate to the **reconciler** subagent using the Task tool:
...
5. **POST-DELEGATION VERIFICATION** (mandatory — subagents may not update
   ...
   d. If findings are P0/P1, consider routing them through the **verifier**
      subagent before acting, same as `/review` step 5 does for reviewer
      findings — a horizontal-audit false positive is exactly as costly as
      a vertical-review one.
