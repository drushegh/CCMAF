...
# via `node tools/console.mjs start`, but only if the MODEL actually executes
...
# SessionStart it backgrounds `tools/console.mjs start` so "open the project"
# alone brings the Console up, no model action required.
#
...
# lives in tools/console.mjs's own `start` verb. This hook does not duplicate
...
[ -f "$ROOT/tools/console.mjs" ] || { echo "{}"; exit 0; }
...
( cd "$ROOT" 2>/dev/null || exit 0
  nohup node tools/console.mjs start >/dev/null 2>&1 &
) &
