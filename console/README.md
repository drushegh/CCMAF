...
the framework's agents read on the next pass. One loopback origin (sole file
writer); a machine-global tray **Hub** opens a small flyout over every running
console. **Personal / local use only** — no auth, no multi-tenancy, no remote
access.
...
- `src/server/` — Fastify server (loopback, sole writer), parsers, `routes/`
